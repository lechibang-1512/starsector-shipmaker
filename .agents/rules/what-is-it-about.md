...
maven_and_environment_quirks:
  rules:
    ...
    - "On Ubuntu/Debian systems, IDE internal terminals (like VS Code) may completely ignore `~/.mavenrc`. The `JAVA_HOME` must be explicitly exported in `~/.bashrc` (e.g., `export JAVA_HOME=/usr/lib/jvm/java-21-openjdk-amd64`) to prevent the environment from defaulting to the wrong Java version and crashing during compilation."
    - "When running `mvn clean compile exec:java`, never execute concurrent background `mvn clean` tasks. Because Maven's `exec:java` loads classes dynamically, a concurrent `clean` goal will purge the `target/classes` directory precisely as the application JVM initializes, throwing a `java.lang.NoClassDefFoundError: shipeditor/persistence/Initializations` and crashing the startup sequence."
    - "The `Main.java` class includes a `checkAndRelaunch(args)` method that automatically forks a new JVM with `-Xmx4g` if available heap is < 4GB. When launching via `mvn exec:java`, `System.getProperty(\"java.class.path\")` only contains the Plexus Launcher. Therefore, Maven itself must be allocated sufficient memory (e.g., via `MAVEN_OPTS=\"-Xmx4g\"`) to avoid triggering this relaunch fallback, which would fail to find the project classpath."
