...
public class InstalledFeatureList extends SortableList<InstalledFeature> {
    ...
    protected JPopupMenu getContextMenu() {
        ...
        oth.shipeditor.components.datafiles.entities.WeaponCSVEntry pickedForInstall = 
                oth.shipeditor.components.viewer.layers.ship.FeaturesOverseer.getWeaponForInstall();
        if (pickedForInstall != null) {
            ...
            replacePoint.addActionListener(event -> actOnSelectedEntry(feature -> {
                ...
                if (slotPainter != null) {
                    var slotPoint = slotPainter.getSlotByID(feature.getSlotID());
                    if (slotPoint != null) {
                        EventBus.publish(new oth.shipeditor.communication.events.viewer.points.PointSelectQueued(slotPoint));
                        EventBus.publish(new oth.shipeditor.communication.events.viewer.control.FeatureInstallQueued(null));
                    }
                }
            }));
            menu.add(replacePoint);
        }
