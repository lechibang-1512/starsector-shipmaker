...
public class EngineSlotPainter extends AngledPointPainter {
    ...
    protected void initInteractionListeners() {
        ...
        // Subscribe to raw mouse events and evaluate predicates locally.
        BusEventListener rawMouseListener = event -> {
            if (!isInteractionEnabled() || !isControlHotkeyPressed()) return;
            if (event instanceof oth.shipeditor.communication.events.viewer.control.ViewerRawMouseDragged checked) {
                ...
                if (ControlPredicates.changeAnglePredicate.test(me)) {
                    ...
                    java.awt.geom.AffineTransform rotatedTransform = oth.shipeditor.utility.overseers.StaticController.getScreenToWorld();
                    Point2D worldTarget = rotatedTransform.transform(me.getPoint(), null);
                    this.handleSizeChange(worldTarget);
                }
            } else if (event instanceof oth.shipeditor.communication.events.viewer.control.ViewerRawMouseMoved checked) {
                java.awt.event.MouseEvent me = checked.mouseEvent();
                if (ControlPredicates.changeAnglePredicate.test(me)) {
                    Point2D worldTarget = computeWorldTarget(me);
                    super.changePointAngleByTarget(worldTarget);
                }
            } else if (event instanceof oth.shipeditor.communication.events.viewer.control.ViewerRawMousePressed checked) {
                java.awt.event.MouseEvent me = checked.mouseEvent();
                if (ControlPredicates.changeAnglePredicate.test(me)) {
                    Point2D worldTarget = computeWorldTarget(me);
                    super.changePointAngleByTarget(worldTarget);
        ...
        listeners.add(rawMouseListener);
        EventBus.subscribe(rawMouseListener);
    ...
    /**
     * Computes the world-space target from a mouse event, respecting rotation rounding.
     */
    private Point2D computeWorldTarget(java.awt.event.MouseEvent me) {
        java.awt.geom.AffineTransform rotatedTransform = oth.shipeditor.utility.overseers.StaticController.getScreenToWorld();
        Point2D target = me.getPoint();
        if (ControlPredicates.isRotationRoundingEnabled()) {
            target = oth.shipeditor.utility.overseers.StaticController.getAdjustedCursor();
        }
