...
public final class UndoOverseer {
    ...
    public static void post(Edit edit) {
        ...
        while (stack.size() > MAX_UNDO_CAPACITY) {
            Edit oldest = stack.removeLast();
            if (oldest instanceof ListeningEdit checked) {
    ...
    private static void markActiveLayerDirty(Edit edit) {
        ...
        if (active instanceof ShipLayer) {
            String className = edit.getClass().getSimpleName();
            String packageName = edit.getClass().getPackageName();
            
            boolean isVariant = packageName.contains(".features") ||
                                className.contains("Hullmod") ||
                                className.contains("Wing") ||
                                className.contains("Feature") ||
                                className.contains("WeaponGroup") ||
                                className.contains("Suppressed") ||
                                className.contains("Variant");
                                
            if (className.contains("Slot") && !className.contains("WeaponGroup")) {
                isVariant = false;
            }
            
            String type = isVariant ? "variant" : "hull";
