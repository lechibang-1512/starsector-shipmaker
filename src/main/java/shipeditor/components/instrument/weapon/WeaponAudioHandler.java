...
public class WeaponAudioHandler {
    ...
    public int populate(JPanel panel, int startRow) {
        int row = startRow;
        ComponentUtilities.addLabelAndComponent(panel, new JLabel(StringManager.getString("FIRE_SOUND_ONE_ID")), fireSoundOneEditor, row++);
        ComponentUtilities.addLabelAndComponent(panel, new JLabel(StringManager.getString("FIRE_SOUND_TWO_ID")), fireSoundTwoEditor, row++);
