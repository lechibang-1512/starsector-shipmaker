    ...
    private static void handleShipEntryDrop(DropTargetDropEvent dtde, ShipCSVEntry shipEntry) {
        try {
            ...
            if (isModulesMode && hasSlots) {
                VariantFile forInstall = null;
                if (StaticController.getActiveLayer() instanceof ShipLayer layer) {
                    forInstall = layer.getFeaturesOverseer().getModuleVariantForInstall();
                }
